...
export const Route = createFileRoute("/backtest")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Valide estratégias com dados históricos de 10+ anos em segundos.",
      },
    ],
  }),
  component: Backtest,
...
function Backtest() {
  ...
  const equity = Array.from(
    { length: 60 },
    (_, i) => 10000 + Math.sin(i * 0.3) * 400 + i * 120 + Math.cos(i * 0.7) * 200,
  );
  const metrics = [
  ...
  return (
    <PageShell
      ...
      title={
        <>
          Teste antes de <span className="gradient-text">arriscar capital</span>
        </>
      }
          ...
          <div className="card-elevated rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-mono text-muted-foreground">
                {done ? "último resultado" : "-"}
              </span>
            </div>
            <div className="h-48">
              <Sparkline data={equity} positive />
            </div>
          </div>
