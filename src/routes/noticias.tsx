...
export const Route = createFileRoute("/noticias")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Feed de notícias macro e de empresas com análise de sentimento por IA.",
      },
    ],
  }),
  component: Noticias,
...
function Noticias() {
  ...
  const color = (s: string) =>
    s === "bull" ? "var(--bull)" : s === "bear" ? "var(--bear)" : "var(--neon)";
  return (
    <PageShell
      eyebrow="Notícias"
      title={
        <>
          Mercado lido por <span className="gradient-text">IA de sentimento</span>
        </>
      }
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto">
          {FILTERS.map((x) => (
            <button
              key={x}
              onClick={() => setF(x)}
              className={`px-4 py-2 rounded-lg text-sm font-medium border ${f === x ? "border-primary text-primary-foreground" : "border-border bg-secondary/40 text-muted-foreground"}`}
              style={f === x ? { background: "var(--gradient-neon)" } : {}}
            >
              {x}
            </button>
          ))}
